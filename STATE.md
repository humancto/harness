...
**Last updated:** 2026-08-24 (post-5.13b history UI)
...
| #65 | 5.13b | History UI (PRD §18.6) + the denial-coalescing follow-up ADR-0041 deferred here. `/history` renders the §10.6 log with the chain banner ABOVE the table, because a hash chain nobody can see verified is decoration: the banner reports exactly what the server checked (`verificationSummary` never upgrades `checked: false` to "verified" — a page with no local rows says so), and a `broken` verdict names the seq. Filters cover all four §18.6 axes but not by one mechanism: `action` and `node` narrow the QUERY (the endpoint indexes them), `actor` prefix and the half-open time window filter the fetched page client-side, and the footer says which count is which rather than implying the page is the whole result. Notable rows (cloud.escalated, shell.denied, secret.accessed) are highlighted and chipped — §18.6 asks for cloud escalations, but what an auditor scans for is the privileged few: work leaving the LAN, a policy refusal, a secret read. Export ships what is ON SCREEN, filters included; `toCsv` quotes every field and doubles inner quotes (a `detail` object is JSON with commas in it). Paging pushes the full compound `(at_ms, node, seq)` cursor 5.13a's diff review established, so a same-millisecond dispatch burst does not swallow the history behind it. STORE SIDE: `shell.denied` is attacker-triggerable at submit rate (`rate_limit` is declared in manifests and enforced nowhere), so an unbounded append lets a peer push genuine entries out of the 100k retention window one denial at a time; `StoreAuditSink` now coalesces floodable actions in a 60s window keyed `(action, subject, actor)`. The repeat cannot bump a counter on the row it repeats — that row is already hashed into the chain — so it is DROPPED and counted, and the count rides the next entry that DOES append as `suppressed_repeats`; the log reads "N suppressed" instead of losing the fact. The window map is in memory (a restart resets it, erring toward RECORDING) and bounded at 1024 keys, so a flooder varying `subject` cannot turn the bookkeeping into its own leak; non-floodable actions are never dropped however repetitive, and a non-object `detail` is nested rather than replaced. +13 tests (3 store, 10 UI; 1236 workspace + 66 UI) |
