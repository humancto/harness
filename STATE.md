...
| #66 | 5.13c-1 | Peer head pins + fork detection (ADR-0041 amendment, Decisions 8–14). The item that starts turning 5.13a's chain into EVIDENCE: a node can rewrite its own DB and its chain still verifies — what it cannot do is un-tell a peer that already pinned `(seq, entry_hash)`. New `harness.audit` QUIC channel (const + `channels::known()` + a `peer_net` recv arm — registering the name without a handler accepts the stream and drops it at the `other =>` fallback, which is how a channel ships inert), `AuditSyncEnvelope` carrying signed `AuditHead`s, V0010 `audit_peer_heads` + `audit_head_conflicts`, and an `AuditSyncService` pushing our head every 30s with the newest third-party heads relayed alongside. FIVE design points, each one a defect avoided: (1) pins are APPEND-ONLY keyed `(node_id, seq)` — "higher seq replaces" deletes the pin the check needs, so a truncate-and-regrow reads as ordinary growth and the ingest of the lie erases the evidence of itself; the PK collision IS the fork detector, and keying on the hash too would store both histories as ordinary pins and detect nothing. (2) A lower-seq head is NOT evidence by itself — the envelope is not `Sequenced`, so any peer can rebroadcast a genuine old head forever and a node returning from a partition does it by accident; "lower = regression" would be a one-packet permanent defamation of an honest node. (3) NO heartbeat field: `Signable::canonical_bytes` re-encodes the DECODED struct, so an added field makes a pre-5.13c node drop the key, re-encode without it, and FAIL signature verification — the heartbeat channel dies and the peer ages out. That is a wire break, not the addition ADR-0019's `replica_head` was assumed to be; a new channel is genuinely additive (unknown name → reset, connection survives). (4) The inner verification key comes from our OWN trust store, never the relaying envelope — a relayer that supplied the key would mint a keypair and forge the history it claims to report; an unkeyable node's head is dropped, because an unverifiable accusation is worse than none. (5) Thinning is status-aware and buckets on `first_seen_ms`: a generic age sweep would eventually evict a `contradicted` pin and reach (1)'s failure by the back door, and bucketing on `seq` would let a flooder push honest history into the tail. Codex round (2 P1 + 1 P2, all verified real, all fixed): (a) every head in ONE envelope is pinned at the same receiver-side instant, so the "newest N" correlated count saw zero newer rows for all of them and exempted every one — a peer sending 64 signed heads per envelope defeated the table bound outright; ties now break on `rowid` (p06c fails against the old SQL: 40 pins, none thinnable). (b) `thin_peer_head_pins` had ZERO production callers — the same defect `audit_prune` shipped with in 5.13a — now wired into the hourly housekeeping and driven by m07 through the housekeeping entry point, not the store method. (c) the backoff could never fire: `send_to` reports only that the message reached the LOCAL queue, and the wire send is asynchronous, so a pre-5.13c peer would get its unknown channel opened and reset every tick forever; failures now come from `sender_task` via `note_send_failed` (the `on_assign_send_failed` precedent), with `note_send_ok` clearing the streak so a transient outage cannot strand a peer. Diff-review round (REVISE→fixed) found two BLOCKERs the Codex round had not. (1) `StaleIgnored` returned WITHOUT inserting, so one validly-signed head at `u64::MAX` permanently immunized its sender: every genuine head afterwards compared below the poisoned maximum, was discarded, and no peer ever pinned that node again — silently, with no removal path. Not treating a low head as EVIDENCE is correct; not STORING it was the defect. A below-newest head is now pinned and classified `StalePinned`, and an oversized seq is REJECTED rather than clamped (clamping collided distinct signed heads onto one row — a fork manufactured by a lossy cast — and made the relayed rebuild carry a seq the signer never signed, so it failed verification at every receiver). (2) The invariant the ADR spends the most words on — the inner key comes from OUR trust store, never the relayer — had ZERO coverage: inverting it to `pubkey_for(from)` left all five daemon tests green, because `boot_pair` structurally cannot express the attack (it needs a trusted relayer B forging a head for a trusted THIRD node C). A three-identity harness now covers both directions, and m08/m09 both fail against the inversion. Also fixed: relay ranking used the head's own `at_ms`, which its signer chooses, so a node stamping `u64::MAX` held a relay slot forever — now ranked on `observed_at_ms`, our clock; `all_peers()` was cloned once PER HEAD (64 clones per envelope) — now one map per envelope; thinning subqueries did not filter on status, so manufactured forks consumed the recent window and accelerated eviction of honest pins; `StalePinned`/`RejectedSeq` were silent — a seq regression is the loudest signal available before entries exist (a wiped or restored DB looks exactly like it), so it warns; m03 built its fork with a direct store call, making it a duplicate of p07 rather than a test of the wire path — it now goes through `ingest`. Re-review round (APPROVE with one MAJOR, fixed before merge): the relay fix had covered cross-node RANKING but left within-node SELECTION on `ORDER BY seq DESC` — and `seq` is exactly as attacker-chosen as `at_ms` was, with `RejectedSeq` stopping only values above `i64::MAX`, so a head signed at exactly `i64::MAX` would be selected as that node's newest pin forever and every honest peer would relay that garbage on its behalf permanently. Selection now orders on `observed_at_ms` first, and the ADR states the general rule 5.13c-2 inherits: no ordering decision may key on a field the subject of that ordering controls. The re-review also caught that m10 passed with the always-insert fix REVERTED (it used `u64::MAX`, which the seq rejection filtered before the insert path was reached) and that the relay-ranking fix had no test at all (reverting the sort left all ten daemon tests green) — m10 now uses a representable poison and m11 covers both orderings, each verified to fail against its own defect. `StalePinned` warns only first-party: a relayed head lagging ours is the normal steady state and warning on it would drown the signal. +23 tests (11 store, 12 daemon; 1268 workspace). HONEST LIMIT, in the ADR: pins alone catch a same-seq fork, NOT truncate-and-regrow-past-the-pin — that needs the entry walk 5.13c-2 adds, so every pin sits at `unchecked` until then and the UI must not render that as corroboration |
